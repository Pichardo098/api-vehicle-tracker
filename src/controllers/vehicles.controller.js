...
exports.createVehicle = catchAsync(async (req, res, next) => {
    ...
    const { model, color, plate_number, brand } = req.body
    ...
    if (plateNumberExists) {
        return next(new AppError('Ya existe un veichulo registrado con el mismo número de placa.', 401))
    }
...
exports.getAllVehicles = catchAsync(async (req, res, next) => {
    ...
    const query = {
        ...
        attributes: ['id', 'user_id', 'model', 'color', 'brand', 'plate_number', 'active', 'createdAt'],
        include: [
            {
                model: Users,
                as: 'user',
